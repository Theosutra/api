"""
Factory Pattern pour la gestion unifiée des fournisseurs LLM.

Ce module centralise la création et la gestion des providers LLM,
offrant une interface unifiée pour tous les fournisseurs supportés.
...
class LLMFactory:
    ...
    async def generate_sql(
        ...
        Returns:
            Requête SQL générée
        """
        # Construction du prompt SQL spécialisé
        prompt = self._build_sql_prompt(user_query, schema, similar_queries or [])
...
SQL:"""
        ...
        if similar_queries:
            ...
            for i, query in enumerate(similar_queries[:3], 1):  # Limiter à 3 exemples
